    ...
    <div class="row center-block mt-50">
        ...
        <div class="col-md-6">
            <div class="text-center bg-white border-rounded box-shadow">
                {{--<!--login form-->--}}
                <img src="{{asset((app()->getLocale()=='ar')?'logo.jpeg':'logo.jpeg')}}" class="mt-50" alt="" width="80px" height="80px" style="padding: 5px; border-radius: 5px">
                @if (session('error'))
                    <div class="alert alert-danger" role="alert">
                        {{ session('error') }}
                    </div>
                @endif
                <form class="mt-50 text-center row" method="POST" action="{{ url('dashboard\login') }}">
                    @csrf
