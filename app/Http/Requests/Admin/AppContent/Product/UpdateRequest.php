...
namespace App\Http\Requests\Admin\AppContent\Product;
...
class UpdateRequest extends FormRequest
{
    ...
    public function preset($crud,$id){
        ...
        //$attributes[] = Media::where('ref_id', $Object->getId())->delete();
        ...
        $attributes[] = Attribute::where('product_id', $Object->getId())->delete();
        foreach ($this->attribute_name as $key=>$attr){
            $Model = new Attribute();
            $Model->setProductId($Object->getId());
            //(app()->getLocale() == 'ar')?$Model->setNameAr($attr):$Model->setName($attr);
            $Model->setName($attr);
            $Model->setNameAr($attr);
            //(app()->getLocale() == 'ar')?$Model->setDescriptionAr($this->attribute_description[$key]):$Model->setDescription($this->attribute_description[$key]);
            $Model->setDescription($this->attribute_description[$key]);
            $Model->setDescriptionAr($this->attribute_description[$key]);
            $Model->setImage(($this->file('attribute_image'))[$key]);
            $Model->save();
        }
        $sizes_prices[] = SizePrice::where('product_id', $Object->getId())->delete();
